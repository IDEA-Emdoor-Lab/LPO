...
MEGATRON_PATH=/cognitive_comp/sunqianguo/workspace/tmp/wangrui/pai-megatron-patch
PYTHONPATH=$(which python)
export PYTHONPATH=${MEGATRON_PATH}:${MEGATRON_PATH}/megatron # PAI-Megatron-LM-240718:$PYTHONPATH
export CUDA_DEVICE_MAX_CONNECTIONS=1
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True

wandb_project="alm-xpo-20250630"
wandb_team="ccnl-s-team"
wandb_exp_name="alm-xpo"

# Here are some configs controled by env
MP_DATASET_TYPE='idx_map'
...
date=20250630
SAVE_INTERVAL=500
DATASET_PATH1=/cognitive_comp/ccnl_common_data/large/sft_audio_data/output/text/${date}/train/mmap_data/text_document

VALID_DATASET_PATH=/cognitive_comp/ccnl_common_data/large/sft_audio_data/output/20250123/tmp2/sft_train.jsonl #/cognitive_comp/sunqianguo/workspace/Pai-Megatron-Patch/qwen-datasets/wudao_qwenbpe_text_document #/cognitive_comp/sunqianguo/workspace/Pai-Megatron-Patch/data/pretrain/output/multi_modal/pretrain_asr_tts_data_content_document #/cognitive_comp/sunqianguo/workspace/Pai-Megatron-Patch/qwen-datasets/wudao_qwenbpe_text_document
step=500
PRETRAIN_CHECKPOINT_PATH=/cognitive_comp/ccnl_common_data/large/sft_audio_data/output/text/20250630/train/checkpoints/checkpoint/xpo-mcore-qwen2.5-7B-lr-2e-7-minlr-1e-7-bs-6-gbs-24-seqlen-4096-pr-bf16-tp-2-pp-4-cp-1-ac-false-do-true-sp-true-ti-33000-wi-66 #/cognitive_comp/ccnl_common_data/wangrui/alm_sft_training/20250606/ref_ckpt/iter_000500_tp2_pp4

# the following two values will not be used when SFT is true
TRAIN_TOKENS=131850000
WARMUP_TOKENS=100000  #1000000000 
###############################

OUTPUT_BASEPATH=/cognitive_comp/ccnl_common_data/large/sft_audio_data/output/text/${date}/train/checkpoints
...
EXTRA_VOCAB_SIZE=421  # 421 + 32768 = 33189
RMS_NORM_EPS=1e-6
gqa_options=" \
		    --group-query-attention \
		    --num-query-groups ${NUM_KEY_VALUE_HEADS}"
...
if [ $XPO = true ]; then
    TRAIN_ITERS=33000
    LR_WARMUP_ITERS=66
    TRAIN_ITERS_DOUBLE=$(expr $TRAIN_ITERS \* 2)
    LR_DECAY_ITERS=$(( ${TRAIN_ITERS_DOUBLE} - ${LR_WARMUP_ITERS}))
    PREFIX="xpo-mcore-qwen2.5-${MODEL_SIZE}-lr-${LR}-minlr-${MIN_LR}-bs-${BATCH_SIZE}-gbs-${GLOBAL_BATCH_SIZE}-seqlen-${SEQ_LEN}"
    sft_option=" \
        --eod-mask-loss \
        --train-mode xpo \
        --loss-type lpo-ste \
