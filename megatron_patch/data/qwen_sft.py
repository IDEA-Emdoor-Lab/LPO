            ...
            if os.path.exists(filename):
                with open(filename) as f:
                    for line in tqdm(f):
                        ...
                        '''
                        position_ids = np.arange(self.max_padding_length)
                        attention_mask = np.zeros(
                            (self.max_padding_length, self.max_padding_length)
                        )
                        ...
                        data_dict["text"] = data_dict["text"] + [self.tokenizer.pad_token_id] * self.max_padding_length
                        data_dict["loss_mask"] = data_dict["loss_mask"] + [self.IGNORE_INDEX] * self.max_padding_length
                        if sum(data_dict["loss_mask"][: self.max_padding_length - 1]) == 0: # 防止截断后mask全为0，导致loss=nan
                            data_dict["loss_mask"][max_padding_length - 2] = 1
                        # item["text"] = item["text"][: max_len]
                        # item["text"][-1] = self.tokenizer.eod_id # 最后一位强行设置成eos
                        sample = {
                            "input_ids": np.array(
                                data_dict["text"][: max_padding_length] + [self.tokenizer.pad_token_id], dtype=np.int64
                            ),
                            "loss_mask": np.array(
                                data_dict["loss_mask"][: max_padding_length], dtype=np.int64
                            ),
                        ...
                        # print(f'sample is:{sample}', flush=True)
                        # import sys
                        # sys.exit(0)
