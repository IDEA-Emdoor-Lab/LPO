...
def get_batch_on_this_tp_rank_idxmap_sft_sample_idx(data_iterator, per_seq_average=False):
    ...
    if mpu.get_tensor_model_parallel_rank() == 0:
        ...
        tokens_concat = torch.cat(
                [tokens, torch.full(data['tokens'].shape[:-1] + (1,), fill_value=tokenizer.pad_token_id, dtype=data['tokens'].dtype)], 
                dim=-1
            )
...
def is_in_loss_mask_range(idx, begin_idx, end_idx):
...
def build_xpo_loss_mask(loss_mask_begin_end: list, seq_len: int):
    loss_mask = []
    for begin_end in loss_mask_begin_end:
        begin = begin_end[0]
        end = begin_end[1]
        loss_mask_t = [1 if is_in_loss_mask_range(i, begin, end) else 0 
                       for i in range(seq_len)]
        loss_mask.append(loss_mask_t)

    return torch.tensor(loss_mask, dtype=torch.float32)


def get_attention_mask_loss_mask_xpo(bs_tokens):
    micro_batch_size, seq_length = bs_tokens.size()
...
def get_batch_on_this_tp_rank_idxmap_xpo(data_iterator, per_seq_average=False):
    args = get_args()
    tokenizer = get_tokenizer()
    def _broadcast(item):
        if item is None:
    ...
    if mpu.get_tensor_model_parallel_rank() == 0:
        ...
        tokens: np.ndarray = data['tokens']
        # sanity check
        # print(f'Actual length {tokens.shape} Setting Length: {2 * (args.seq_length + 1 + 3)} Data Type: {tokens.dtype}')
        assert tokens.shape[-1] == 2 * (args.seq_length + 1 + 3)
        # print(f"data['tokens'] shape is:{tokens.shape}", flush=True)

        start_time = time.time()

        actual_seqlen = args.seq_length + 1
        chosen_input_ids = tokens[:, :actual_seqlen].long()
        chosen_logprob = tokens[:, actual_seqlen + 2]
        chosen_loss_mask_begin_end = tokens[:, actual_seqlen: actual_seqlen + 2].long().tolist()
        chosen_loss_mask = build_xpo_loss_mask(loss_mask_begin_end=chosen_loss_mask_begin_end,
                                               seq_len=args.seq_length + 1)

        reject_begin_idx = args.seq_length + 1 + 3
        reject_ids_end_idx = reject_begin_idx + args.seq_length + 1
        reject_input_ids = tokens[:, reject_begin_idx: reject_ids_end_idx].long()
        reject_logprob = tokens[:, reject_ids_end_idx + 2]
        reject_loss_mask_begin_end = tokens[:, reject_ids_end_idx: reject_ids_end_idx + 2].long().tolist()
        reject_loss_mask = build_xpo_loss_mask(loss_mask_begin_end=reject_loss_mask_begin_end,
                                               seq_len=args.seq_length + 1)
        
        input_ids = torch.cat([chosen_input_ids, reject_input_ids], dim=0).long()
        loss_mask = torch.cat([chosen_loss_mask, reject_loss_mask], dim=0)
        ref_log_probs = torch.cat([chosen_logprob, reject_logprob], dim=0).float().view(-1, 1)

        '''input_ids = chosen_input_ids.long() 
        loss_mask = chosen_loss_mask 
        ref_log_probs = chosen_logprob.view(-1, 1)'''

        tokens = input_ids[..., :-1].contiguous()
        labels = input_ids[..., 1:].contiguous()
        loss_mask = loss_mask[:, :-1].contiguous()
        micro_bsz, _ = tokens.shape
        sample_idx_list =  [[0, args.seq_length]] * micro_bsz
        sample_idx_list = torch.tensor(sample_idx_list, dtype=torch.int64)
        
        #  loss_mask = (labels != -100).float()
        attention_mask, position_ids = get_attention_mask_loss_mask_xpo(tokens)
        end_time = time.time()
        # print(f'total time spend in make data:{end_time-start_time}', flush=True)
        ...
        if SHOW_DATA:
            ...
            print_rank_0("tokens: {}, shape: {}".format(tokens[0], tokens.shape))
            print_rank_0("labels: {}, shape: {}".format(labels[0], labels.shape))
            print(f'Log probs: \n{ref_log_probs}, shape: {ref_log_probs.shape}')
            print_rank_0(
                "attention_mask: {}, shape: {}".format(
                    attention_mask[0], attention_mask.shape
                )
            )
            print_rank_0(f"loss_mask: {loss_mask[0]} shape: {loss_mask.shape}")
            print_rank_0(f"position_ids: {position_ids[0]} shape: {position_ids.shape}")
        ...
        # dtype: long, long, float, bool, long, float
        batch = {
            'tokens': tokens.cuda(non_blocking=True),
            'labels': labels.cuda(non_blocking=True),
            'loss_mask': loss_mask.cuda(non_blocking=True),
            'attention_mask': attention_mask.cuda(non_blocking=True) if attention_mask is not None else None,
            'position_ids': position_ids.cuda(non_blocking=True),
            'ref_logprobs': ref_log_probs.cuda(non_blocking=True),
            'num_seqs': num_seqs.cuda(non_blocking=True) if num_seqs is not None else None
        }
        ...
        # dtype: long, long, float, bool, long, float
        # xpo_micro_bsz = args.micro_batch_size * 2
        xpo_micro_bsz = args.micro_batch_size
        xpo_seq_length = args.seq_length
        tokens = torch.empty((xpo_micro_bsz, xpo_seq_length), dtype=torch.int64,
                             device=torch.cuda.current_device())
        labels = torch.empty((xpo_micro_bsz, xpo_seq_length), dtype=torch.int64,
                             device=torch.cuda.current_device())
        loss_mask = torch.empty((xpo_micro_bsz, xpo_seq_length), dtype=torch.float32,
                                device=torch.cuda.current_device())
        ref_log_probs = torch.empty((xpo_micro_bsz, 1), dtype=torch.float32,
                                    device=torch.cuda.current_device())
        
        attention_mask = None
        if args.create_attention_mask_in_dataloader:
            mbs = args.micro_batch_size if args.reset_attention_mask else 1
            attention_mask = torch.empty((xpo_micro_bsz, 1, xpo_seq_length, xpo_seq_length), dtype=torch.bool,
                                        device=torch.cuda.current_device())
        position_ids = torch.empty((xpo_micro_bsz, xpo_seq_length), dtype=torch.int64,
                                   device=torch.cuda.current_device())

        num_seqs = None
        if per_seq_average:
            num_seqs = torch.empty((xpo_micro_bsz,), dtype=torch.int64,
                                    device=torch.cuda.current_device()) 
