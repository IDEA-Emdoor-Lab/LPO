...
def load_megatron_model(args):
    ...
    if (
        ...
        layers_to_copy = {}
        for tp_rank in tqdm(range(args.tensor_model_parallel_size), desc='Loading megatron params'):
            for pp_rank in range(args.pipeline_model_parallel_size):
                ...
                for k, v in split_state.items():
                    try:
                        ...
                        mid_state[k].append(v)
        for k, v in tqdm(mid_state.items(), desc='Converting megatron params'):
            if not isinstance(v[0], torch.Tensor) or 'norm' in k:
                target_v = v[0]
            elif 'embedding' in k or 'output_layer' in k:
                target_v = torch.cat(v, dim=0)
...
def convert_checkpoint_from_megatron_to_transformers(mgmodel, hfmodel, args):
    print('Converting float to half-float...', flush=True)
    if args.fp16:
    ...
    print('Precision converting done.')
    ...
    with torch.no_grad():
        print(f'Megatron word embedding size: {mgmodel.embedding.word_embeddings.weight.size()}', flush=True)
        print(f'HF word embedding size: {hfmodel.model.embed_tokens.weight.size()}', flush=True)
        hfmodel.model.embed_tokens.weight.copy_(mgmodel.embedding.word_embeddings.weight)
        for mglayer, hflayer in tqdm(zip(mgmodel.decoder.layers, hfmodel.model.layers), desc='Converting Layers'):
            if use_te:
...
def main():
    ...
    if args.convert_checkpoint_from_megatron_to_transformers:
        ...
        print('Loading megatron params, done.', flush=True)
