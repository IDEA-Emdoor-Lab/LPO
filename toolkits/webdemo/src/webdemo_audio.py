...
class AudioWebDemo(object):
    ...
    def format_system(self, system_prompt: str,
        ...
        if task_mode == "general":
            ...
            '''
            if user_audio1:
                user_audio1_text = self.enocde_audio(user_audio1)
                system_str = system_str.replace('<user_audio1>', '<|inter_audio_begin|>' + user_audio1_text + '<|inter_audio_end|>')
            if user_audio2:
                user_audio2_text = self.enocde_audio(user_audio2)
                system_str = system_str.replace('<user_audio2>', '<|inter_audio_begin|>' + user_audio2_text + '<|inter_audio_end|>')

            system_str += role_prompt
            if role_audio1:
                role_audio1_text = self.enocde_audio(role_audio1)

                system_str = system_str.replace('<role_audio1>', '<|inter_audio_begin|>' + role_audio1_text + '<|inter_audio_end|>')
            if role_audio2:
                role_audio2_text = self.enocde_audio(role_audio2)
                system_str = system_str.replace('<role_audio2>', '<|inter_audio_begin|>' + role_audio2_text + '<|inter_audio_end|>')
            '''
            system_str += role_prompt
    ...
    def create_demo(self):
        ...
        with gr.Blocks(title="LAM Demo", fill_height=True) as demo:
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    with gr.Column(scale=2):
                        ...
                        with gr.Tab('role_dialog') as role_dialog_tab:
                            user_prompt = gr.Textbox(label="用户人设",
                            ...
                            role_prompt = gr.Textbox(label="模型人设",
...
if __name__ == "__main__":
    ...
    # @app.post(args.api_url)
    #async def chat(query=Body(None), clear_history=Body(False)):
    #    output = mind_bot.inference_fn(query, clear_history)
    #    return output
    ...
    #            ssl_certfile="/cognitive_comp/sunqianguo/workspace/GLM-4-Voice/ssl/cert.pem", ssl_keyfile="/cognitive_comp/sunqianguo/workspace/GLM-4-Voice/ssl/key.pem", ssl_verify=False)
    #          ssl_certfile="/cognitive_comp/sunqianguo/workspace/pai-megatron-patch/toolkits/webdemo/ssl/cert.pem", ssl_keyfile="/cognitive_comp/sunqianguo/workspace/pai-megatron-patch/toolkits/webdemo/ssl/key.pem", ssl_verify=False)
